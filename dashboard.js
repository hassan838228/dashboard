...
const hcaptcha_secret = '0x554309872F7CBdd05Ec13fBfC036D8A8586b073B826c42da-093b-4f92-a708-216c04f91a84';
const token = 'aca7ca9b-5152-414b-9ca6-87b552e48296';
...
module.exports = async (client) => {
      ...
      for (let i = 0; db.questions.length > i; i++) {
        if (data[i + 1]) {
          ...
          embed = new MessageEmbed()
            ...
            .setFooter({ text: "https://c-bot.click/" })
            .setColor("GREEN");
        } else {
          form.paste.push(`Question #${i + 1} - ${db.questions[i]}`);
      ...
      member
        .send({
          embeds: [
            new MessageEmbed()
              .setColor("GREEN")
              .setFooter({ text: `Powered by https://c-bot.click` })
  ...
  // Developer endpoint - protected route for authorized developers
  app.get("/developer", checkAuth, (req, res) => {
    // Check if user is a developer (using the developers array from config)
    if (!jsonconfig.developers.includes(req.user.id)) {
  ...
  app.get("/dashboard/:guildID", checkAuth, async (req, res) => {
    ...
    renderTemplate(res, req, "./new/mainpage.ejs", {
      guild: guild,
      alert: `Dashboard was made by https://c-bot.click`,
 ...
 */
  ...
  app.post("/dashboard/:guildID/tickets", checkAuth, async (req, res) => {
    ...
    if (
      ...
      // Handle new ticket settings
      if (data.clientCloseHides !== undefined) {
        ticketSettings.clientCloseHides = data.clientCloseHides === 'true';
      }

      if (data.ticketReceivedEnabled !== undefined) {
        ticketSettings.ticketReceivedEnabled = data.ticketReceivedEnabled === 'true';
      }

      if (data.inquiryEnabled !== undefined) {
        ticketSettings.inquiryEnabled = data.inquiryEnabled === 'true';
      }

      if (data.showOnlineAdmins !== undefined && storedSettings.isPremium === 'true') {
        ticketSettings.showOnlineAdmins = data.showOnlineAdmins === 'true';
      }
...
In the mean time, please explain your issue below`;
    ...
    // Handle new premium and advanced settings
    if (Object.prototype.hasOwnProperty.call(data, "advancedUpdate")) {
      // Client close behavior
      if (data.clientCloseBehavior === "hide") {
        ticketSettings.clientCloseHides = true;
      } else if (data.clientCloseBehavior === "delete") {
        ticketSettings.clientCloseHides = false;
      }

      // Inquiry button
      ticketSettings.inquiryEnabled = data.inquiryEnabled ? true : false;

      // Premium features (only if premium)
      if (storedSettings.isPremium === "true") {
        ticketSettings.ticketReceivedEnabled = data.ticketReceivedEnabled ? true : false;
        ticketSettings.showOnlineAdmins = data.showOnlineAdmins ? true : false;
      }
    ...
    if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
      ...
      if (ticketSettings.ticketToggle == "true") {
        if (
          ...
          if (storedSettings.isPremium == "false") {
            ticketSettings.ticketFooter = "Powered by cBot.cBot";
          } else {
          ...
          let reactionFooter = "Powered by cBot.cBot";

          let footer = "Powered by cBot.cBot";
          // eslint-disable-next-line no-unused-vars
          ...
          if (storedSettings.isPremium == "false") {
            ticketEmbed.setFooter({ text: `Powered by cBot.cBot` });
          } else {
            if (checkFooter) {
  ...
  app.post("/dblwebhook", webhook.middleware(), async (req) => {
    ...
    const msg = new Discord.MessageEmbed()
      ...
      .setDescription(
        `Thank you **${apiUser.username}#${apiUser.discriminator}** (${apiUser.id}) for voting **cBot**!`
      );
    Hook.sendCustom(msg);
